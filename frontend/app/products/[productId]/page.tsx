    ...
    <div className="space-y-5">
      ...
      <Card>
        ...
        <p className="mt-1 text-xs text-sub">
          いずれも最新のPO明細（PO発行日が最新の1件）、または全ての仕入明細（カラー/サイズ違いやリピートオーダーを含む全行）から取得。発注単価・予定輸入経費率・予定原価単価はPO入力時点（最新1件）、実績輸入経費率・実績原価単価は仕入確定後の値（全明細行の加重平均）です。発注単価・PO履歴の金額は、POに入力された通貨のままで、円ではない場合があります。予定原価単価・実績原価単価は、いずれも円換算済みの値です。
        </p>
      </Card>
            ...
            {product.sales.length > 0 ? (
              product.sales.map((s, idx) => (
                <div key={idx} className="rounded-lg border border-slate-200 p-3 text-xs">
                  ...
                  <p className="mt-1 text-sub">売上伝票ID: {s.ID ?? "—"} ・ カラー: {s.カラー ?? "—"} ・ サイズ: {s.サイズ ?? "—"} ・ 営業: {s.営業担当者名 ?? "—"} ・ 数量: {s.数量pcs ?? "—"} ・ 金額: {fmtYen(s.売上金額)}</p>
                </div>
              ))
            ) : (
            ...
            {product.purchases.length > 0 ? (
              product.purchases.map((p, idx) => (
                <div key={idx} className="rounded-lg border border-slate-200 p-3 text-xs">
                  ...
                  <p className="mt-1 text-sub">
                    仕入伝票ID: {p.ID ?? "—"} ・ カラー: {p.カラー ?? "—"} ・ サイズ: {p.サイズ ?? "—"} ・ 営業: {p.営業担当者名 ?? "—"} ・ 生産管理: {p.生産管理担当者名 ?? "—"} ・ 数量: {p.仕入数量pcs ?? "—"} ・ 金額: {fmtYen(p.仕入金額円)}
                  </p>
                </div>
              ))
            ) : (
