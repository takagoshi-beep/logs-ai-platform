...
export default async function HomePage() {
  ...
  const displayTodayActions = homeData.today_actions && homeData.today_actions.length > 0
    ? homeData.today_actions.map((action, idx) => ({
        id: `action-${idx}`,
        project: "System",
        status: action.priority || "info",
        title: action.title || "",
          ...
          <ul className="mt-4 space-y-3">
            {displayTodayActions.map((item) => (
              <li key={item.id} className="surface-soft p-3">
                ...
                {item.source_table && <p className="text-xs text-gray-400 mt-0.5">Source: {item.source_table}</p>}
              </li>
            ))}
          </ul>
        </Card>
        ...
        <div className="space-y-4">
          ...
          <ActionPanel
            ...
            items={[
              {
                label: "Used Data",
                value: homeData.data_sources?.join(", ") || "Sales trend, task logs, project timeline"
              },
