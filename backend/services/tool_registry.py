...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "recordsが切り捨てられるため、それだけで顧客ごとの合計を"
    ...
    },
    {
        ...
        "description": (
            ...
            "件数・数量範囲・経費率の範囲（最小・推定=中央値・最大）・推定金額を集計する。"
            "【重要・2026-09-09、Noritsuguの指摘】「関税とその他の内訳を教えて」のように、"
            "直前の試算結果についての追加質問をされた場合、get_purchase_surchargesを"
            "呼んではいけない（このツールが既に`推定関税円`・`推定運賃等円`・"
            "`関税率_平均`を返しているため、get_purchase_surchargesで生の諸掛明細を"
            "別途調べ直す必要はない）。会話履歴には自分の過去の回答テキストしか残らず、"
            "ツールが返した生データそのものは保持されないため、前回と同じパラメータ"
            "（quantity・unit_price_usd・category_code）でこのツールを再度呼び出し、"
            "実際の数値を取得してから答えること。「(実績関税率を適用)」のような、"
            "計算方法の説明文をあたかも数値であるかのように空欄のまま表示してはいけない"
            "— 必ず実際に取得した数値そのものを示すこと。"
            "諸掛込原価（推定・実績とも）は関税・運賃等の諸掛を含むが、消費税は含まない"
            "（2026-09-08、Noritsuguの指摘。「消費税を含む」のように、このツールが"
            "返していない項目を含んでいるかのように事前に説明してはいけない）。"
            "【重要・2026-09-09、Noritsuguの指摘】`推定輸入経費円`・`推定諸掛込原価円`は"
            "関税とその他の諸掛を分けて算出している: 関税（`推定関税円`）は商品の申告価格に"
            "比例する性質があるため、実際に関税額が記録されている伝票（purchase_surcharges"
            "の実データ、DDP等で関税0円と記録されている伝票は除く）の`関税率_平均`×想定"
            "商品原価。運賃・通関手数料等の「その他の諸掛」（`推定運賃等円`）は商品価値に"
            "左右されない固定費部分のため、実績データの1個あたり実額（中央値）×想定数量"
            "（14.133・14.134）。`推定経費率`はこの結果から逆算した参考表示用の値であり、"
            "計算の起点ではない。`関税データあり伝票数`が0の場合、関税分は0として扱われる"
            "（`関税データあり伝票数`自体は内部的な判断材料であり、表には含めなくてよい）。"
            "為替レートは実際の直近の仕入データから取得する（架空の為替レートは使わない）。"
            "【重要】このツールは呼び出し時に輸送方法を指定する必要が無い — 1回の呼び出しで"
            "FEDEX・DHL・FERRY等、実データに存在する輸送方法全ての内訳を一括で返す。"
            "「FEDEXの場合」のように特定の輸送方法だけを聞かれた場合も、このツールをそのまま"
            "呼び出し、返ってきた`records`の中からその輸送方法の行を選んで提示すればよい"
            "（「この組み合わせ専用のツールが無い」という理由で使うのをやめて、"
            "get_purchase_linesのような別のツールで少数の実例を手動で探しに行っては"
            "いけない — 経費率のばらつきを実データの分布として正しく見せられなくなる）。"
            "【使い方】get_purchase_linesで少数の実例を選んで散文で外挿するのではなく、"
            "このツールが返す集計結果（records、輸送方法ごとの行）をそのまま提示すること。"
            "【表示形式・2026-09-09、Noritsuguの指定】箇条書きではなく、必ずMarkdownの"
            "表形式で提示すること（見やすさのため）。「過去の実績データそのもの」と"
            "「今回の質問（数量・単価）に対する最終的な見積もり結果」を混同しやすいため、"
            "輸送方法ごとに必ず次の2つの表に分けて提示すること（1つの表に混ぜない）。"
            "表1「実績サマリー」（過去のデータそのもの、推定値は含めない）: "
            "輸送方法・伝票数・対象数量（`対象数量_平均`個、`対象数量_最小`〜"
            "`対象数量_最大`）・経費率範囲（`経費率_最小`〜`経費率_最大`）・"
            "`関税率_平均`・実績1個あたり原価範囲（`実績1個あたり原価_最小円`〜"
            "`_最大円`）・実績1個あたり諸掛込原価範囲（`実績1個あたり諸掛込原価_最小円`〜"
            "`_最大円`）・主な仕入先。"
            "表2「推定結果」（今回の質問に対する最終的な見積もり数字のみ）: "
            "輸送方法・推定関税円・推定運賃等円（運賃・通関料等、"
            "`推定関税円`・`推定運賃等円`は合算した`推定輸入経費円`1列にまとめず、"
            "必ず別々の列として示すこと）・推定輸入経費円・推定諸掛込原価円・"
            "`推定経費率`（諸掛込原価÷商品原価。Noritsuguが見積もりに"
            "そのまま使用するため必須）・推定1個あたり原価円。"
            "`推定輸入経費率`（輸入経費だけを商品原価に対する比率）というフィールドも"
            "結果に含まれているが、`推定経費率`と重複するため表2には含めないこと"
            "（2026-09-10、Noritsuguの指定）。"
            "【重要・2026-09-10、Noritsuguの指定】経費率（表1の経費率範囲・"
            "表2の`推定経費率`）は、数値のみで表示し「倍」という単位を"
            "付けないこと（例: 「1.198」であり「1.198倍」ではない）。"
            "一方、`関税率_平均`は百分率（%）で表示すること（例: フィールドの値が"
            "0.0959なら「9.59%」と表示する。小数のまま「0.0959」と表示しない）。"
            "`対象数量`（数量の情報）を省略してはいけない。"
            "【重要・2026-09-08、Noritsuguの指摘】経費率という比率だけでは実際の商品原価・"
            "輸入経費の実額が分からず、誤った経費率を採用するリスクがある（経費率は本来、"
            "運賃・関税等の実額から決まるものであり、比率は概算のための簡易的な手段に"
            "すぎない）。規模の異なる伝票をそのまま合計しても比較材料として意味が薄いため、"
            "1個あたりに正規化した実績の範囲を必ず表1に含めること。"
            "【重要・2026-09-08、Noritsuguの指摘】各行の`仕入先別内訳`は、同じ輸送方法内でも"
            "仕入先ごとに経費率が大きく異なりうることを示す（取引条件がDDP＝仕入先が関税・"
            "輸送費を商品代金に既に含めて請求する場合、経費率が低く出るのが正常であり、"
            "データ不備ではない）。`経費率_最小`〜`経費率_最大`の幅が大きい場合は、必ず"
            "`仕入先別内訳`（表3として、表1・表2とは別に提示すること）も一緒に提示すること"
            "（仕入先名から国籍・取引条件を作り話してはいけない — `仕入先別内訳`に表れている"
            "経費率の違いだけを事実として伝える）。"
            "【重要】想定単価（`想定単価USD`）と、それを基に計算した商品原価（`商品原価円`）は、"
            "必ず回答の冒頭または表に明示すること（質問者が見積もりの前提が妥当か判断できる"
            "ようにするため）。同じ条件の実データから算出した`実績平均単価USD_参考`も必ず"
            "併記し、想定単価と大きく異なる場合はその旨を正直に伝えること — この参考値は"
            "ツール自身が計算して返す値であり、別のツールで事前に調べる必要は無い。"
            "ユーザーが単価を指定しなかった場合、Claudeが単価を推測で決めてこのツールを"
            "呼んではいけない — その場合はこのツールを呼ばずに、具体的な単価をユーザーに"
            "直接尋ねること（単価が分かった時点で、このツールを1回呼び出せばよい。単価を"
            "確認するために別のツールを先に呼ぶ必要は無い）。"
            "各行の`主な仕入先`に無い属性（仕入先の国籍・所在地等）を作り話してはいけない"
            "（取得したデータに含まれていない情報を推測で付け加えないこと）。"
            "`データ不足`がTrueの行は伝票数が3件未満のため、参考値である旨を必ず伝えること。"
            "NEWHATTANブランドの仕入先は既定で除外される（include_newhattan=Trueで含める）。"
            "【重要・2026-09-08、Noritsuguの指摘】`推定経費率`・`推定1個あたり原価円`等、"
            "`推定`と付いている値は全てあくまで見積もり・試算であり、確定した金額ではない。"
            "「推奨」という、能動的に薦めているかのような誤解を招く表現は使わないこと"
            "（実データから機械的に算出した中央値であり、この輸送方法を推奨しているわけ"
            "ではない）。表の見出しや文中で「試算」「見積」等に言い換えず、必ず「推定」"
            "という言葉に統一すること。"
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "unit_price_usd": {
                    "type": "number",
                    "description": "想定単価（USD）。ユーザーが明示した値のみを渡すこと。ユーザーが単価を指定していない場合、Claudeが推測で数値を作って渡してはいけない。",
                },
                "category_code": {
                    "type": "integer",
    ...
    },
    {
        ...
        "description": (
            ...
            "納品予定期間で検索する。各行に営業担当者名・営業事務担当者名・生産管理"
    ...
    },
    {
        ...
        "description": (
            "仕入先（工場）ごとの平均納期・平均発注単価を算出する。"
            "「〇〇工場の平均納期は？」「どの仕入先が一番早い/遅い？」"
            "「〇〇に頼むといくらくらい？」のような質問に使う。"
            ...
            "`平均発注単価内訳`は輸入経費（輸送方法・為替・関税等）を含まない、"
            "工場側の見積もり単価そのもの（発注ごとに変動する輸入経費を含めると"
            "「工場自体の単価はいくらか」という問いへの答えとしてはノイズになるため、"
            "意図的に除外している）。通貨ごとに分けて算出されるため、異なる通貨の"
            "単価を混同して比較・合算してはいけない。"
        ),
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": (
            ...
            "【必ず呼ぶべき質問】質問に「予算」「予定」「見込み」「費用」"
            "「経費」「達成率」「フォーキャスト」のいずれかが含まれる場合は、"
            "他のツールだけで済ませず必ずこのツールも呼び出すこと。特に"
            "「〇〇さんの今後の売上予定」「顧客ごとの予定」のように、"
            "顧客・担当者単位の見込みを聞かれた場合は、このツールを最優先で"
            "呼ぶこと（get_projectsの「未納品PO」だけで済ませて『予定は0件』"
            "と結論づけてはいけない）。"
            "【「予定」の二重の意味に注意】この会社では「予定」に2つの"
            "別々の意味がありうる: (1) 確定済み受注(PO)でまだ納品していない"
            "もの＝get_projectsで分かる実績ベースの未来、(2) このテーブルの"
            "category=forecast(02_予定)＝営業担当者が見積もった顧客ごとの"
            "見込み数値（PO化前でもよい）。質問がどちらを指しているか曖昧な"
            "場合は、両方のツールを呼んだ上で、それぞれ何を意味するかの"
            "違いを説明してから提示すること（片方だけ調べて『予定はありません』"
            "と断定しない）。"
            "1行=1案件（顧客×担当者×期×月×分類）。categoryは budget=予算、"
            "forecast=売上予定、expense=費用の3種類のみ（発注・実績データは"
            "ここには無い — 発注はget_projects、売上実績はget_sales_lines"
            "を使うこと）。"
    ...
    },
    {
        ...
        "description": (
            "指定した生産担当が扱っているサンプル依頼を、仕入先・商品名とともに取得する"
            "（「通知状況」は現在使われていない機能のため、絞り込み・結果のいずれにも"
            "使わず、全件を返す）。"
            "各行の`sp_planned_eta`（サンプル専用の到着予定日）・`sp_planned_etd`（同、出荷予定日）"
            "は、対応中サンプルの約15〜18%にしか"
    ...
    },
    {
        ...
        "description": (
            ...
            "【重要】各行のdays_until_deliveryは、正式な"
    ...
    },
    {
        ...
        "description": (
            ...
            "気づいた場合に呼び出す。"
            "「データが存在しない/見つからない」（＝そのケースはget_XXXツール"
            "が返すstatus:\"unavailable\"で十分）とは異なり、こちらは「絞り込み・"
            "集計の手段自体がツールに用意されていない」ケース専用（例:"
            "「今月のOEMの売上は？」で、事業分類による絞り込み手段が"
            "get_sales_linesにもget_sales_by_categoryにも無かったようなケース）。"
            "呼び出しても回答自体は変わらない — 正直に「現状のツールでは"
            "できません」と説明した上で、この不足を記録するために呼ぶ。"
            "本当に機能不足の場合のみ呼ぶこと（データが単に無い/0件だった"
            "だけの場合には呼ばない）。"
            "【必ず呼ぶべきケース】ツールの結果に\"truncated\": trueが含まれて"
            "おり、かつ集計・ランキング・件数などをその不完全なrecordsから"
            "自分で計算して回答した場合は、必ずこのツールを呼ぶこと。"
        ),
        "input_schema": {
            "type": "object",
            "properties": {
