...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "【重要・2026-09-08、Noritsuguの指摘】想定単価（`想定単価USD`）と、それを基に"
            "計算した商品原価（`商品原価円`）は、必ず回答の冒頭または表に明示すること"
            "（質問者が見積もりの前提が妥当か判断できるようにするため）。同じ条件の実データ"
            "から算出した`実績平均単価USD_参考`も必ず併記し、想定単価と大きく異なる場合は"
            "その旨を正直に伝えること（想定単価が実態とかけ離れていると、以降の推定金額も"
            "意味を持たない）。ユーザーが単価を指定しなかった場合、Claudeが単価を推測で"
            "決めてこのツールを呼んではいけない — 想定単価が無いと計算できない旨を伝え、"
            "具体的な単価を尋ねるか、`実績平均単価USD_参考`に相当する値を一度別途確認して"
            "から、それを想定単価として明示した上で呼び出すこと。"
