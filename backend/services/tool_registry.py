...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "「想定商品原価×経費率（比率）」ではなく、「実績データの輸入経費の実額"
            "（1個あたり、中央値）×想定数量」を想定商品原価に加算する方式で算出している"
            "（関税・運賃等には商品価値に左右されない固定費部分があるため、想定単価が実績"
            "データと大きく異なる場合、比率を掛ける方式では推定額が不自然に歪む）。"
            "`推定経費率`はこの結果から逆算した参考表示用の値であり、計算の起点ではない。"
