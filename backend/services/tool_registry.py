...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "表形式で提示すること（見やすさのため）。列は必ず次の順序にすること: "
            "(1) 実績に関する列を左側にまとめる — 輸送方法・伝票数・対象数量（"
            "`対象数量_平均`個、`対象数量_最小`〜`対象数量_最大`）・経費率範囲（"
            "`経費率_最小`〜`経費率_最大`）・主な仕入先。"
            "(2) 推定値に関する列を右側にまとめる — 推定経費率・`推定輸入経費率`"
            "（諸掛込原価÷商品原価の`推定経費率`とは異なり、輸入経費だけを商品原価に"
            "対する比率として取り出した値。見積もりに使う場合に必要、2026-09-09、"
            "Noritsuguの指定）・推定関税円・"
            "推定その他諸掛円（運賃・通関料等、`推定関税円`・`推定その他諸掛円`は"
            "合算した`推定輸入経費円`1列にまとめず、必ず別々の列として示すこと）・"
            "推定諸掛込原価・推定1個あたり原価。"
            ...
            "1個あたりに正規化した実績の範囲（`実績1個あたり原価_最小円`〜`_最大円`・"
            "`実績1個あたり諸掛込原価_最小円`〜`_最大円`）を、「輸送方法別」の表にも"
            "「仕入先別内訳」の表にも、経費率と一緒に必ず列として含めること。"
            ...
            "`仕入先別内訳`も一緒に提示すること（仕入先名から国籍・取引条件を作り話しては"
            "いけない — `仕入先別内訳`に表れている経費率の違いだけを事実として伝える）。"
