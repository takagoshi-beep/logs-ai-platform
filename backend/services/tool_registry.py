...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "呼んではいけない（このツールが既に`推定関税円`・`推定その他諸掛円`・"
            ...
            "商品原価。運賃・通関手数料等の「その他の諸掛」（`推定その他諸掛円`）は商品価値に"
            ...
            "輸送方法・推定関税円・推定その他諸掛円（運賃・通関料等、"
            "`推定関税円`・`推定その他諸掛円`は合算した`推定輸入経費円`1列にまとめず、"
            "必ず別々の列として示すこと）・推定輸入経費円・推定諸掛込原価円・"
            "`推定経費率`（諸掛込原価÷商品原価、1.XXX倍の表現。Noritsuguが見積もりに"
            "そのまま使用するため必須）・推定1個あたり原価円。"
            "`推定輸入経費率`（輸入経費だけを商品原価に対する比率）というフィールドも"
            "結果に含まれているが、`推定経費率`と重複するため表2には含めないこと"
            "（2026-09-10、Noritsuguの指定。`推定経費率`の1.XXX倍という表現だけに統一する）。"
