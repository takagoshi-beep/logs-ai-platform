...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "「輸入経費率」は諸掛込原価÷商品原価（商品単価×数量×為替）を示す指標で、1.xxの"
            "値になる（例: 1.25なら諸掛が25%上乗せされている）。各行の`経費率`列、および"
            "`aggregate.輸入経費率`は既にこの定義で計算済みの値なので、`仕入金額円`・"
            "`諸掛込金額円`からパーセンテージ等を自分で計算し直してはいけない"
            "（実例: 25.5%という独自のマークアップ率計算をして、本来の1.xxという定義と"
            "異なる表記になってしまった事例があった）。"
            ...
            "各行には`輸送方法`（伝票単位の配送方法）・`通貨`/`通貨名`（1=USD, 2=円, "
            "3=RMB）・`為替`（適用為替レート）も含まれる。輸入経費率の変動要因を説明する"
            "際は、これらの実在する列の値を実際に見て言及すること（実例: 「輸送方法に"
            "よって経費率が変動する」と、実際にはその列を見ずに述べてしまった事例が"
            "あった。取得したrecordsに含まれていない要因を推測で挙げてはいけない）。"
