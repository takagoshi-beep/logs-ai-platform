...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "と結論づけてはいけない — 2026-07-14、実際にこの見落としが"
            "発生した実例あり）。"
    ...
    },
    {
        ...
        "description": (
            ...
            "（2026-07-15、14.108で「通知状況」による絞り込みを撤去 — 「通知完了」は見積"
            "回答を依頼元に伝え終えたという工程の完了を示すだけで、その後もサンプル到着"
            "自体は追跡対象でありうると確認されたため、通知状況に関わらず全件を返す。"
            "各行のnotification_statusフィールドで通知状況自体は確認できる）。"
    ...
    },
    {
        ...
        "description": (
            ...
            "気づいた場合に呼び出す（2026-07-13、14.82、14.89でルール明確化）。"
            ...
            "自分で計算して回答した場合は、必ずこのツールを呼ぶこと（2026-07-13、"
            "顧客ランキングをget_sales_linesの切り捨てられたデータから作って"
            "しまったにも関わらず、このツールを呼ばなかった実例あり）。"
        ),
        "input_schema": {
            "type": "object",
            "properties": {
