...
class LogsysProvider:
    ...
    def _import_cost_estimate(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        return _evidence(
            ...
            f"提示すること。箇条書きや文章だけで済ませてはいけない。列は実績に関する列"
            f"（輸送方法・伝票数・対象数量・経費率範囲・主な仕入先）を左側、推定値に関する"
            f"列（推定経費率・`推定輸入経費率`（諸掛込原価÷商品原価の推定経費率とは異なり、"
            f"輸入経費だけを商品原価に対する比率として取り出した値。見積もりに使う場合に"
            f"必要）・推定関税円・推定その他諸掛円・推定諸掛込原価・推定1個あたり"
            f"原価）を右側の順にすること。`推定関税円`・`推定その他諸掛円`は合算した"
            f"`推定輸入経費円`1列にまとめず、必ず別々の列として示すこと。",
            results,
        )
