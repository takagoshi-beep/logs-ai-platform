...
class LogsysProvider:
    ...
    def _import_cost_estimate(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        # 経費率の下限が1.018倍のように、一般的な関税・輸送費等を考えると
        # 現実的に低すぎる値が混ざっていた。原因は、"諸掛込金額円" >
        # "仕入金額円"（経費率が1.0より大きい）というチェックだけでは、
        # 諸掛（輸入経費）の入力が途中の伝票（例: 関税だけ入力済みで運賃・
        ...
        # 正しくは、実際に記録された関税額（purchase_surchargesの諸掛
        # 区分ID=6、"関税合計円"）を使い、
        # 関税とその他の諸掛を分けて算出する。
        #
        # 2026-09-09（14.140、Noritsuguが実データで発見）: 関税は
        # purchase_surchargesに「伝票単位」の値として記録されている
        # （14.138参照）が、1つの伝票に複数の商品分類の明細が混在する
        # 場合（実データで確認: 全伝票の約15%、ベルトを含む伝票では
        # 約20%が複数分類混在）、この関税額はその伝票の全商品分類の
        # 合計仕入金額に対してかかったものであり、特定の商品分類だけの
        # 仕入金額（"合計仕入金額円"、商品分類で絞り込んだ明細だけの
        # 合計）とは対応しない。これを知らずに関税率＝関税合計円／
        # （絞り込み後の）合計仕入金額円としてしまうと、分母が実態より
        # 小さくなり、関税率が異常に高く算出されてしまう（実例:
        # 本来1割程度のはずが63%になっていた）。"経費率"（諸掛込金額円／
        # 仕入金額円）自体は明細レベルで既に按分済みの値と考えられる
        # ため大きな問題にはならないが、関税だけは伝票単位のため、
        # 商品分類が単一の伝票（他の商品分類の明細を含まない伝票）だけ
        # を関税率の計算対象にする。
        sql = (
            'WITH voucher_agg AS ('
            ...
            '), '
            'voucher_purity AS ('
            '  SELECT "伝票番号", (COUNT(DISTINCT "商品分類") = 1) AS "商品分類が単一" '
            '  FROM purchases '
            '  WHERE "ステータス" IN (2, 3) AND "仕入金額円" > 0 '
            '  GROUP BY "伝票番号"'
            ') '
            'SELECT v."伝票番号", v."輸送方法", v."仕入先名", v."合計数量pcs", v."合計仕入金額円", '
            '       v."合計諸掛込金額円", COALESCE(t."関税合計円", 0) AS "関税合計円", '
            '       COALESCE(vp."商品分類が単一", FALSE) AS "商品分類が単一", '
            '       v."合計諸掛込金額円" / v."合計仕入金額円" AS "経費率" '
            'FROM voucher_agg v '
            'LEFT JOIN tariff_agg t ON v."仕入ID" = t."仕入ID" '
            'LEFT JOIN voucher_purity vp ON v."伝票番号" = vp."伝票番号" '
        ...
        for transport_code, group in by_transport.items():
            ...
            # 2026-09-09（14.140、Noritsuguが実データで発見）: 関税は
            # 伝票単位の値のため、複数の商品分類が混在する伝票では、
            # 絞り込み後の"合計仕入金額円"（この商品分類だけの明細の
            # 合計）を分母にすると、実際にはこの商品分類以外の仕入分も
            # 含めて課された関税を、この商品分類だけにかかったものと
            ...
            results.append({
                ...
                # 2026-09-09（14.141、Noritsuguの指定）: 「推定経費率」
                # （諸掛込原価÷商品原価、例1.196）は商品原価を含んだ
                # 倍率であり、輸入経費だけを取り出した比率が別途欲しいと
                # の要望。`推定輸入経費円`÷`想定商品原価円`（＝推定経費率
                # から1.0を引いた値と等価）を独立したフィールドとして返す。
                "推定輸入経費率": (
                    round(est_cost / buy_jpy, 3) if buy_jpy else None
                ),
                "仕入先別内訳": supplier_breakdown,
                "推定仕入金額円": round(buy_jpy),
                "推定関税円": round(est_tariff),
                "推定運賃等円": round(est_other),
        ...
        return _evidence(
            ...
            f"DDP等で関税0円と記録されている伝票、および他の商品分類の明細が混在する伝票"
            f"（関税は伝票単位の値のため、混在伝票では対象の商品分類だけにかかった関税額を"
            f"特定できない）は除く）から算出した`関税率_平均`×想定商品原価。"
            f"(2) 運賃・通関手数料等の「その他の諸掛」（`推定運賃等円`）は商品価値に"
            ...
            f"ではない）。表の見出しや文中で「試算」「見積」等の言い換えをせず、必ず"
            f"「推定」という言葉に統一すること。"
            f"【重要・2026-09-08、Noritsuguの指摘】諸掛込原価（`推定諸掛込原価円`・"
            f"`実績1個あたり諸掛込原価`等）は関税・運賃等の諸掛を含むが、消費税は含まない。"
            f"「消費税を含む」のように、この関数が返していない項目を含んでいるかのように"
            f"説明してはいけない。"
            f"【重要・2026-09-09、Noritsuguの指摘】「関税とその他の内訳を教えて」のような、"
            f"この結果についての追加質問をされた場合、get_purchase_surchargesではなく、"
            f"同じパラメータでこのツールを再度呼び出すこと（`推定関税円`・`推定運賃等円`・"
            f"`関税率_平均`が既にこの結果に含まれている）。「(実績関税率を適用)」のような"
            f"計算方法の説明文を、実際の数値の代わりに空欄のまま表示してはいけない。"
            f"【表示形式・再確認・2026-09-09、Noritsuguの指定】必ずMarkdownの表形式で"
            f"提示すること。箇条書きや文章だけで済ませてはいけない。「過去の実績データ"
            f"そのもの」と「今回の質問に対する最終的な見積もり結果」を混同しやすいため、"
            f"輸送方法ごとに必ず次の2つの表に分けて提示すること（1つの表に混ぜない）。"
            f"表1「実績サマリー」（推定値は含めない）: 輸送方法・伝票数・対象数量・"
            f"経費率範囲・`関税率_平均`・実績1個あたり原価範囲・実績1個あたり諸掛込原価"
            f"範囲・主な仕入先。表2「推定結果」（今回の質問に対する最終的な数字のみ）: "
            f"輸送方法・推定関税円・推定運賃等円・推定輸入経費円・推定諸掛込原価円・"
            f"`推定経費率`（Noritsuguが見積もりにそのまま使用するため必須）・"
            f"推定1個あたり原価円。`推定輸入経費率`というフィールドも結果に含まれているが、"
            f"`推定経費率`と重複するため表2には含めないこと（2026-09-10、Noritsuguの指定）。"
            f"経費率（表1の経費率範囲・表2の`推定経費率`）は数値のみで"
            f"表示し「倍」という単位を付けないこと（例:「1.198」であり「1.198倍」ではない）。"
            f"一方、`関税率_平均`は百分率（%）で表示すること（例: フィールドの値が0.0959なら"
            f"「9.59%」と表示する。小数のまま「0.0959」と表示しない）。"
            f"`推定関税円`・`推定運賃等円`は合算した`推定輸入経費円`1列にまとめず、必ず別々の列"
            f"として示すこと。`関税データあり伝票数`は内部的な判断材料であり、表には"
            f"含めなくてよい。",
            results,
        )
    ...
    def fetch(self, dataset: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if dataset == "ongoing_samples_by_staff":
            ...
            return _evidence(
                ...
                "全件を返す。「通知状況」自体は現在使われていない機能のため、結果には含めていない）。"
