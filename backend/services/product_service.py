...
def get_product_detail(product_id: str) -> dict[str, Any] | None:
    ...
    # 通貨コードも一緒に持たせる。
    #
    # 2026-07-13（14.84、Noritsuguが実データで発見）: 当時、"売上原価"も
    # "発注単価"と同じ行の同じ"通貨"列に属する値だと考え、"発注単価通貨"
    # と同じ値を"予定原価単価通貨"としても持たせていた。
    #
    # 2026-07-15（14.113、Noritsuguが実データで発見・修正）: 実際には
    # "売上原価"（→"予定原価単価"の元になる列）は常に円換算済みの値
    # だったと判明した（実例: 発注単価19.8 USD、輸入経費率1.15のPOで
    # 予定原価単価が3,416と表示されており、19.8×約150円/USD×1.15≒3,416
    # と一致 — 円換算後の数値と分かる。"合計発注金額"/"合計売上原価"/
    # "合計売上金額"がヘッダーレベルで既に円換算済みであるのと同じ理由で、
    # 明細レベルの"売上原価"も円換算済みだったと考えられる）。14.84の
    # 「発注単価と同じ通貨」という前提はこの列には当てはまらなかった
    # ため、"予定原価単価通貨"は常に"円"固定に修正する
    # （"発注単価"・"発注単価通貨"自体はUSD等の元の通貨のままで正しい
    # ため変更しない）。
    ...
    master["予定原価単価通貨"] = "円" if master["予定原価単価"] is not None else None
